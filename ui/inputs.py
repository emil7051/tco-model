...
def create_input_sidebar():
    ...
    with st.sidebar.expander("General", expanded=True):
        ...
        st.number_input(
            ...
            format="%d", 
            key="start_year",
            value=params.get('start_year', datetime.datetime.now().year) # Default from state or current year
        )
        st.number_input(
            "Life Cycle (Years)", 
            min_value=1, 
        ...
        st.text_area("Description", key="description", value=params.get('description', ''))
    ...
    with st.sidebar.expander("Economic", expanded=False):
        ...
        st.number_input("Discount Rate (%)", min_value=0.0, max_value=20.0, step=0.1, format="%.1f", key="discount_rate", help="Real discount rate (0-20%).", value=params.get('discount_rate', 7.0))
        st.number_input("Inflation Rate (%)", min_value=0.0, max_value=10.0, step=0.1, format="%.1f", key="inflation_rate", help="General inflation rate (0-10%).", value=params.get('inflation_rate', 2.5))
        ...
        if st.session_state['financing_method'] == 'loan': 
            ...
            st.number_input("Loan Interest Rate (%)", min_value=0.0, max_value=20.0, step=0.1, format="%.1f", key="interest_rate", help="Loan interest rate (0-20%).", value=params.get('interest_rate', 7.0))
            st.number_input("Down Payment (%)", min_value=0.0, max_value=100.0, step=1.0, format="%.1f", key="down_payment_pct", help="Down payment percentage (0-100%).", value=params.get('down_payment_pct', 20.0))


    with st.sidebar.expander("Operational", expanded=True):
        st.number_input("Annual Mileage (km)", min_value=0.0, step=1000.0, format="%.0f", key="annual_mileage", value=params.get('annual_mileage', 40000.0))


    with st.sidebar.expander("Electric Vehicle", expanded=True):
        st.text_input("EV Name/Model", key="electric_vehicle_name", value=params.get('electric_vehicle_name', 'EV Default'))
        st.number_input("EV Purchase Price (AUD)", min_value=0.0, step=1000.0, format="%.0f", key="electric_vehicle_price", value=params.get('electric_vehicle_price', 400000.0))
        st.number_input("EV Battery Capacity (kWh)", min_value=0.0, step=1.0, format="%.1f", key="electric_vehicle_battery_capacity", value=params.get('electric_vehicle_battery_capacity', 300.0))
        st.number_input("EV Energy Consumption (kWh/km)", min_value=0.0, step=0.01, format="%.2f", key="electric_vehicle_energy_consumption", value=params.get('electric_vehicle_energy_consumption', 0.7))
    ...
    with st.sidebar.expander("Diesel Vehicle", expanded=True):
        st.text_input("Diesel Name/Model", key="diesel_vehicle_name", value=params.get('diesel_vehicle_name', 'Diesel Default'))
        st.number_input("Diesel Purchase Price (AUD)", min_value=0.0, step=1000.0, format="%.0f", key="diesel_vehicle_price", value=params.get('diesel_vehicle_price', 200000.0))
        st.number_input("Diesel Fuel Consumption (L/100km)", min_value=0.0, step=0.1, format="%.1f", key="diesel_vehicle_fuel_consumption", value=params.get('diesel_vehicle_fuel_consumption', 28.6))
        # Residual value is now loaded from projections - display info or omit input
    ...
    with st.sidebar.expander("Battery Replacement (EV)", expanded=False):
        ...
        if st.session_state['enable_battery_replacement']:
            ...
            if replace_mode == "Fixed Year":
                ...
                start_year = params.get('start_year', datetime.datetime.now().year)
                ...
                st.number_input("Battery Replacement Year (Index)", 
                                ...
                                value=params.get('battery_replacement_year', 8) # Default replacement year index
                                )
