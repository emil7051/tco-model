...
def create_input_sidebar():
    ...
    with st.sidebar.expander("General", expanded=True):
        ...
        st.number_input("Start Year", step=1, format="%d", key="start_year")
        st.number_input("End Year", step=1, format="%d", key="end_year")
    ...
    with st.sidebar.expander("Economic", expanded=False):
        ...
        st.number_input("Discount Rate (%)", min_value=0.0, max_value=20.0, step=0.1, format="%.1f", key="discount_rate", help="Real discount rate (0-20%).", value=params.get('discount_rate', 0)*100) # Display as percentage
        st.number_input("Inflation Rate (%)", min_value=0.0, max_value=10.0, step=0.1, format="%.1f", key="inflation_rate", help="General inflation rate (0-10%).", value=params.get('inflation_rate', 0)*100) # Display as percentage
        ...
        if st.session_state.scenario_params['financing_method'] == 'loan':
            ...
            st.number_input("Loan Interest Rate (%)", min_value=0.0, max_value=20.0, step=0.1, format="%.1f", key="interest_rate", help="Loan interest rate (0-20%).", value=params.get('interest_rate', 0)*100) # Display as percentage
            st.number_input("Down Payment (%)", min_value=0.0, max_value=100.0, step=1.0, format="%.1f", key="down_payment_pct", help="Down payment percentage (0-100%).", value=params.get('down_payment_pct', 0)*100) # Display as percentage
    ...
    with st.sidebar.expander("Electric Vehicle", expanded=True):
        ...
        st.number_input("EV Residual Value (%)", min_value=0.0, max_value=100.0, step=1.0, format="%.1f", key="electric_residual_value_pct", help="Residual value as % of purchase price (0-100%).", value=params.get('electric_residual_value_pct', 0)*100) # Display as percentage
    ...
    with st.sidebar.expander("Diesel Vehicle", expanded=True):
        ...
        st.number_input("Diesel Residual Value (%)", min_value=0.0, max_value=100.0, step=1.0, format="%.1f", key="diesel_residual_value_pct", help="Residual value as % of purchase price (0-100%).", value=params.get('diesel_residual_value_pct', 0)*100) # Display as percentage
    ...
    with st.sidebar.expander("Battery Replacement (EV)", expanded=False):
        ...
        if st.session_state.scenario_params['enable_battery_replacement']:
            ...
            if replace_mode == "Fixed Year":
                st.number_input("Battery Replacement Year", min_value=params['start_year'], max_value=params['end_year'], step=1, key="battery_replacement_year")
            else: # Capacity Threshold
                # Use .get() for safety, provide default 0, multiply by 100 for display
                initial_threshold_display = (params.get('battery_replacement_threshold', 0) or 0) * 100
                st.number_input("Battery Capacity Threshold (%)", min_value=0.0, max_value=100.0, step=1.0, format="%.1f", key="battery_replacement_threshold", help="Replace when capacity drops below this % (0-100%).", value=initial_threshold_display) # Display as percentage
